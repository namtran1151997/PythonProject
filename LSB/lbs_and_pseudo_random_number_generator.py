...
class Engine:
    ...
    def __init__(self, image, text, key):
        ...
        self.pixels = self.image.load()  # Loads the pixels of the image
        ...
        self.size = self.image.size  # size of the image (x*y)
        self.generator = NumberGenerator(self.size, self.len_txt, self.key)  # initialize a number generator
        self.pixel_list = self.generator.get_pixel_list()  # the list of coordinates of pixels to use to hide or recover
    ...
    def hide(self, output="out.jpg"):
        ...
        for t in text:
            for bit in t:
                if k == 0:
                    x, y = self.pixel_list.pop(0)
                compo = bin(self.pixels[x, y][k])
                compo = list(compo)
                compo.pop()
                compo.append(bit)
                compo = ''.join(compo)
                compo = int(compo, 2)
                if k == 0:
                    self.pixels[x, y] = (compo, self.pixels[x, y][1], self.pixels[x, y][2])
                    k += 1
                elif k == 1:
                    self.pixels[x, y] = (self.pixels[x, y][0], compo, self.pixels[x, y][2])
                    k += 1
                else:
                    self.pixels[x, y] = (self.pixels[x, y][0], self.pixels[x, y][1], compo)
                    k = 0
    ...
    def recover(self, output):
        ...
        :return:
        Extract the text from the image using the key and write it in a file
        if an output file is given or print it if not.
        """
        text = []
        final_text = []
        for x, y in self.pixel_list:
            for k in range(3):
                text.append(self.pixels[x, y][k])
